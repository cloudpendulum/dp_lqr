...
class SymbolicDoublePendulum:
    ...
    def __init__(
        ...
        # Actuator selection Matrix (New)
        # if torque_limit[1] == 0:  # Only joint 1 actuated (pendubot)
        #     self.B_sym = smp.Matrix([[1], [0]])  # Shape: (2,1)
        #     self.B = np.array([[1], [0]])        # 2x1 matrix
        # elif torque_limit[0] == 0:  # Only joint 2 actuated (acrobot)
        #     self.B_sym = smp.Matrix([[0], [1]])
        #     self.B = np.array([[0], [1]])
        # else:  # Fully actuated double pendulum
        #     self.B_sym = smp.Matrix([[1, 0], [0, 1]])
        #     self.B = np.array([[1, 0], [0, 1]])
