...
def plotEllipse(x0, x1, s0Idx, s1Idx, rho, S, ax):
...
def plotEllipseSections(x, rho, S, save_to=None, show=True):
    fig, axs = plt.subplots(3,2, figsize=(10,10))
    plotEllipse(x[0], x[1], 0, 1, rho, S, axs[0][0])
    axs[0][0].set_xlabel("shoulder pos [rad]")
    axs[0][0].set_ylabel("elbow pos [rad]")
    plotEllipse(x[0], x[2], 0, 2, rho, S, axs[1][0])
    axs[1][0].set_xlabel("shoulder pos [rad]")
    axs[1][0].set_ylabel("shoulder vel [rad/s]")
    plotEllipse(x[0], x[3], 0, 3, rho, S, axs[2][0])
    axs[2][0].set_xlabel("shoulder pos [rad]")
    axs[2][0].set_ylabel("elbow vel [rad/s]")
    plotEllipse(x[1], x[2], 1, 2, rho, S, axs[0][1])
    axs[0][1].set_xlabel("elbow pos [rad]")
    axs[0][1].set_ylabel("shoulder vel [rad/s]")
    plotEllipse(x[1], x[3], 1, 3, rho, S, axs[1][1])
    axs[1][1].set_xlabel("elbow pos [rad]")
    axs[1][1].set_ylabel("elbow vel [rad/s]")
    plotEllipse(x[2], x[3], 2, 3, rho, S, axs[2][1])
    axs[2][1].set_xlabel("shoulder vel [rad/s]")
    axs[2][1].set_ylabel("elbow vel [rad/s]")
    plt.tight_layout()
    if not (save_to is None):
        plt.savefig(save_to)
    if show:
